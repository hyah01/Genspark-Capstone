...
import com.genspark.cart_service.controller.CartOrderController;
import com.genspark.cart_service.model.CartOrder;
import com.genspark.cart_service.services.CartOrderImpl;
...
public class CartOrderControllerTest {

    @Mock
    private CartOrderImpl service;
    ...
    void testAddCartOrder() throws Exception {
        ...
        CartOrder cartOrder = new CartOrder("1", "1", "1", 1);
        // Convert Cart Order object to JSON string
        ObjectMapper mapper = new ObjectMapper();
        String cartJson = mapper.writeValueAsString(cartOrder);
        // Mock service behavior
        Mockito.when(service.addCartOrder(Mockito.any(CartOrder.class))).thenReturn(cartOrder);
        ...
        verify(service, times(1)).addCartOrder(Mockito.any(CartOrder.class));
    }

    @Test
    void testUpdateCartOrder() throws Exception {
        CartOrder cartOrder = new CartOrder("1", "1", "1", 1);
        CartOrder updatedCartOrder = new CartOrder("1", "1", "1", 2);
        // Convert Cart Order object to JSON string
        ObjectMapper mapper = new ObjectMapper();
        String cartJson = mapper.writeValueAsString(updatedCartOrder);

        Mockito.when(service.getCartOrderById("1")).thenReturn(cartOrder);
        Mockito.when(service.updateCartOrder(Mockito.any(CartOrder.class))).thenReturn(updatedCartOrder);
        ...
        verify(service, times(1)).updateCartOrder(Mockito.any(CartOrder.class));
    }
    ...
    void testFindAllCartOrders() throws Exception {
        List<CartOrder> cartOrders = List.of(new CartOrder("1", "1", "1", 1),
                new CartOrder("2", "1", "2", 2));
